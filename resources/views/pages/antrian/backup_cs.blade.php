        ...
        <p style="text-align: center;">
          ...
          @else
            @if ($antrian_user->status == "off")
                <a href="{{ url('antrian/page_cs/' . Auth::user()->master_karyawan_id . '/on') }}" class="btn btn-danger">Komputer OFF</a>
            @else
                <a href="{{ url('antrian/page_cs/' . Auth::user()->master_karyawan_id . '/off') }}" class="btn btn-success">Komputer ON</a>
            @endif
          @endif --}}
        </p>
      </div>
...
<script>
    ...
    nomorAntrian();
		...
		function nomorAntrian(timestamp) {
			$('.data-nomor').empty();
			$.ajax({
				url: "{{ URL::route('antrian_cs.nomor') }}",
				type: 'GET',
                      ...
                      '<div class="card-footer">';
                        if (value.status == 0) {
                          val_nomor_antrian += '' +
                            '<div class="d-flex justify-content-center">' +
                              '<a href="page_cs/' + value.nomor_antrian + '/panggil" class="btn btn-primary" style="width: 50px;" title="Panggil"><i class="fas fa-phone"></i></a>' +
                            '</div>';
                        }
                        if (value.status == 1) {
                          if (value.customer_filter_id == 2) {
                            ...
                            if (user_karyawan_id == value.karyawan.id) {
                              val_nomor_antrian += '' +
                                '<div class="d-flex justify-content-between">' +
                                  '<a href="page_cs/' + value.nomor_antrian + '/panggil" class="btn btn-primary" style="width: 50px;" title="Panggil"><i class="fas fa-phone"></i></a>' +
                                  '<a href="page_cs/' + value.nomor_antrian + '/mulai" class="btn btn-success" style="width: 50px;" title="Mulai"><i class="fas fa-play"></i></a>' +
                                  '<a href="page_cs/' + value.nomor_antrian + '/batal" class="btn btn-danger" style="width: 50px;" title="Batal"><i class="fas fa-times"></i></a>' +
                                '</div>';
                            }
                          }
                        }
                        if (value.status == 2) {
                          if (user_karyawan_id == value.karyawan.id) {
                            val_nomor_antrian += '' +
                              '<div class="d-flex justify-content-between">' +
                                '<a href="page_cs/' + value.nomor_antrian + '/selesai" class="btn btn-success" style="width: 50px;" title="Selesai"><i class="fas fa-check"></i></a>' +
                                '<a href="page_cs/' + value.nomor_antrian + '/pindah" class="btn btn-primary" style="width: 50px;" title="Pindah"><i class="fas fa-share"></i></a>' +
                              '</div>';
                          }
                        }
                        val_nomor_antrian += '' +
